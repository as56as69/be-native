...
export function createTransitRouter(deps: TransitDeps = {}): Router {
  ...
  router.post("/start", async (req, res, next) => {
    try {
      ...
      const mode = input.mode === "fast" ? "fast" : "slow";
      const db = getDb();

      if (mode === "fast") {
        const { data: balance } = await db.rpc("spend_credits", {
          p_user_id: input.user_id,
          p_amount: FAST_CREDIT_COST,
        });
        if (balance === null) {
          res.status(402).json({ error: "INSUFFICIENT_CREDITS", cost: FAST_CREDIT_COST });
          return;
        }
      ...
      const gateResult = gate.request(input.user_id, input.from_spot_id, getSlowGateMs());
